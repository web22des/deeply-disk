...
5.  **Проверь данные:**
    - Валидация `id`(как опция),
    - Импорт изображений через `import`

---

## Нейминг компонентов

Так как в данном шаблоне разрабатываются готовые секции для последующего использования в других проектах, следует придерживаться определенного нейминга.
...
- [ ] Поле `id: string` (опционально, если это карточки товара, kebab-case)-
...
| --- | --- | --- |
| Папка компонента | `сamelCase` | `@ui/cardVar02/, sections/Hero/` |
